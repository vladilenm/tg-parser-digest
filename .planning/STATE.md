...
| 260520-i5v | Dashboard send доступен из run-once скриптов. Dashboard build+send вынесен из `tick()` в `buildAndSendDashboard(runId)` в `src/dashboard.ts` (никогда не throw'ит — внутренний try/catch + `sendAlert(stage="dashboard")`). Вызов добавлен в `scripts/run-once.ts` и `scripts/run-once-web.ts` после соответствующего pipeline. Теперь `npm run start:once` и `npm run start:once:web` тоже отправляют dashboard в канал, а не только daemon в cron-время. tsc clean, 2450/2450 тестов зелёные. | 2026-05-20 | a75f42d | [260520-i5v-reuse-dashboard-send-from-run-once-scrip](./quick/260520-i5v-reuse-dashboard-send-from-run-once-scrip/) |
