...
/**
 * H5X-01: builds the static dashboard HTML and writes it to
 * ${DATA_DIR}/dashboard/index.html. Returns the absolute path + byte size.
 *
 * Library function — used by both `npm run dashboard` (CLI shim) and
 * `src/run.ts:tick()` (auto-send after pipelines).
 *
 * Throws if buildData() finds an empty volume — caller decides how to react
 * (CLI shim exits 1; tick() catches → sendAlert + continues).
 */
