...
**Действие:**
- **Канал стал приватным** → удалить из `channels.json` ИЛИ перевести в карантин (см. `docs/CHANNELS.md`).
- **Username не существует** (опечатка/удалили) → удалить запись из `channels.json`.
- **User-аккаунт не подписан** → подписаться руками с того же аккаунта, чей TG_SESSION в .env.
- `pm2 restart tg-parser` после правки channels.json.
